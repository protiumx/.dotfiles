...
map({ 'n', 'v' }, '<Leader>qd', function()
  ...
  local lstart, lend = utils.get_lines_indexes()
  local new_items = {}
  for i, item in ipairs(items) do
    if i < lstart or i > lend then
  ...
  local new_idx = math.min(lstart, #new_items)
...
map(
  'v',
  '<C-y>',
  '"' .. utils.system_clip_reg .. 'y',
  { silent = true, desc = 'Junk into reg ' .. utils.system_clip_reg }
)
...
map('n', '<Leader>P', function()
  ...
  vim.fn.setreg(utils.system_clip_reg, path)
  print('Copied: ' .. path)
end, { desc = 'Copy current path to reg ' .. utils.system_clip_reg })
map('n', '<Leader>L', function()
  local path = vim.fn.expand('%:~:.') .. ':' .. vim.fn.line('.')
  vim.fn.setreg(utils.system_clip_reg, path)
  print('Copied: ' .. path)
end, { desc = 'Copy current path with line and column to reg ' .. utils.system_clip_reg })
map({ 'n', 'v' }, '<Leader>C', function()
  utils.copy_with_context()
end, { desc = 'Copy selection with context', silent = true })
