...
local function go_result_type(info)
  ...
  for _, node in query:iter_captures(function_node, 0) do
    print('iter caps', vim.inspect(node))
...
local go_ret_vals = function(args)
  ...
  local func_name = args[2][1]
  return snippet_from_nodes(
    nil,
    c(nil, {
      t(err_name),
      t(string.format('errors.Wrap(%s, "%s")', err_name, func_name)),
    })
  )
end
...
<finish>
]],
          {
            ...
            args = i(4),
            err_same = rep(2),
            result = d(5, function(args)
              local name = (args and args[1] and args[1][1]) or ''
              print(name)
              return sn(
                nil,
                c(1, {
